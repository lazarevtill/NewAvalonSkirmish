/**
 * @file Renders a modal for the host to assign players to teams.
 */
...
const TeamSlot: React.FC<{
    ...
    isOver: boolean;
}> = ({ teamId, teamName, players, capacity, onDrop, onDragOver, onPlayerDragStart, isOver }) => (
    <div
...
/**
 * A modal that allows the host to assign players to teams using drag-and-drop.
 * @param {TeamAssignmentModalProps} props The properties for the component.
...
export const TeamAssignmentModal: React.FC<TeamAssignmentModalProps> = ({ players, gameMode, onCancel, onConfirm }) => {
    ...
    const { team1Capacity, team2Capacity } = useMemo(() => {
        if (gameMode === GameModeEnum.TwoVTwo) return { team1Capacity: 2, team2Capacity: 2 };
        if (gameMode === GameModeEnum.ThreeVOne) return { team1Capacity: 3, team2Capacity: 1 };
    ...
    const handleDrop = (e: React.DragEvent, targetTeamId: number | 'unassigned') => {
        ...
        if (idxUnassigned > -1) newUnassigned.splice(idxUnassigned, 1);
        const idxTeam1 = newTeam1.findIndex(p => p.id === draggedPlayerId);
        if (idxTeam1 > -1) newTeam1.splice(idxTeam1, 1);
        const idxTeam2 = newTeam2.findIndex(p => p.id === draggedPlayerId);
        if (idxTeam2 > -1) newTeam2.splice(idxTeam2, 1);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-gray-900 border border-gray-700 rounded-lg p-8 shadow-xl w-full max-w-4xl">
                <h2 className="text-3xl font-bold mb-2">Assign Teams</h2>
                <p className="text-gray-400 mb-6">Drag and drop players into their teams.</p>
                <div className="grid grid-cols-3 gap-6">
                    {/* Unassigned Players */}
                    <div
                        onDrop={(e) => handleDrop(e, 'unassigned')}
                        onDragOver={(e) => { handleDragOver(e); setDragOverTarget('unassigned'); }}
                    ...
                    >
                        <h3 className="text-lg font-bold mb-3 border-b border-gray-700 pb-2">Unassigned Players</h3>
                    ...
                    <TeamSlot
                        ...
                        onDrop={handleDrop}
                        onDragOver={(e) => { handleDragOver(e); setDragOverTarget(1); }}
                    ...
                    <TeamSlot
                        ...
                        onDrop={handleDrop}
                        onDragOver={(e) => { handleDragOver(e); setDragOverTarget(2); }}
                        onPlayerDragStart={handleDragStart}
